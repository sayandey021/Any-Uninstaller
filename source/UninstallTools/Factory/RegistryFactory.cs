...
namespace UninstallTools.Factory
{
    public class RegistryFactory : IUninstallerFactory
    {
        ...
        {
            var dateString = uninstallerKey.GetStringSafe(RegistryNameInstallDate)?.Trim();
            if (string.IsNullOrEmpty(dateString))
                return DateTime.MinValue;

            // Try standard DateTime parsing first (e.g. "01/05/2024", "2024-01-05", "2024/01/05")
            if (DateTime.TryParse(dateString, CultureInfo.InvariantCulture, DateTimeStyles.None, out var parsedDate) ||
                DateTime.TryParse(dateString, CultureInfo.CurrentCulture, DateTimeStyles.None, out parsedDate))
            {
                if (parsedDate.Year >= 1970 && parsedDate.Year <= DateTime.Now.Year + 1)
                    return parsedDate;
            }
            ...
            if (dateString.Length == 8)
            {
                try
                {
                    // Standard Windows Installer YYYYMMDD format
