...
namespace UninstallTools.Factory
{
    public class StoreAppFactory : IIndependantUninstallerFactory
    {
        private static string HelperPath
        {
            get
            {
                var candidate1 = Path.Combine(UninstallToolsGlobalConfig.AssemblyLocation, @"StoreAppHelper.exe");
                if (File.Exists(candidate1)) return candidate1;

                var candidate2 = Path.Combine(AppContext.BaseDirectory, @"StoreAppHelper.exe");
                if (File.Exists(candidate2)) return candidate2;

                var candidate3 = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"StoreAppHelper.exe");
                return candidate3;
            }
        }
