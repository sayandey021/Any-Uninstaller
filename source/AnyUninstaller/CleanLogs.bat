...
REM Wait for AnyU to exit so the logs are written
echo Waiting for AnyUninstaller to exit...
:loop
tasklist | find "AnyUninstaller.exe " >nul
if not errorlevel 1 (
...
del /f /s AnyUninstaller.exe.log SteamHelper.exe.log StoreAppHelper.exe.log UninstallerAutomatizer.exe.log UpdateHelper.exe.log
...
REM Remove all directories related to AnyU
for /d %%G in ("%SETTINGSDIR%\AnyUninstaller*") do rd /s /q "%%~G"
